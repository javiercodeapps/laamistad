...
class PaymentProvider(models.Model):
    ...
    def set_qr_info(self):
        ...
        if self.pos_id:
            ...
            self.mp_qr_url = data.get("qr", {}).get("template_document", "")
            qr_image_url = data.get("qr", {}).get("image")
            if qr_image_url:
                response = requests.get(qr_image_url, timeout=REQUEST_TIMEOUT)
            ...
            self.write(
                {
                    "mp_external_pos_id": data.get("external_id", ""),
                    "pos_id": str(data.get("id", "")),
                    "mp_qr_url": data.get("qr", {}).get("template_document", ""),
                    "mp_qr_image": image,
                }
            )
