...
class PaymentMercadopagoPoint(models.Model):
    ...
    def create_order_link_multi(self, sales):
        """Create a single MP payment link for multiple sale orders.
        Args:
            sales: recordset of sale.order
        Returns:
            dict with 'link' (URL) and 'orders' (sale order names)
        """
        mp_token = self.payment_provider_id.mercado_pago_qr_access_token
        base_url = self.get_base_url()
        sdk = mercadopago.SDK(mp_token)
        ...
        for so in sales:
            for line in so.order_line:
                if line.product_uom_qty > 0:
                    mercadopago_items.append({
                        "title": '%s - %s' % (so.name, line.product_id.name),
                        "quantity": line.product_uom_qty,
                        "currency_id": 'ARS',
                        "unit_price": line.price_unit
                    })

        # Store all order names as external_reference (comma separated)
        external_refs = ','.join(sales.mapped('name'))
        ...
        data = {
            ...
            "description": 'Pago múltiple: %s' % external_refs,
        }
        _logger.info("create_order_link_multi data: %s", data)
        preference_response = sdk.preference().create(data)
        _logger.info("create_order_link_multi preference_response: %s", preference_response)
        preference = preference_response["response"]

        # Create history records for each sale order
        for so in sales:
            self.env['payment_mercadopago_point.mercadopago.history'].create({
                'sale_id': so.id,
                'name': so.name,
            ...
            so.message_post(body='Mercado Pago Link de pago múltiple: %s' % preference['init_point'])

        return {
            'link': preference['init_point'],
            'orders': external_refs,
            'total': total_amount,
        }
