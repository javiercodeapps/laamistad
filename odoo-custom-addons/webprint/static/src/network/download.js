...
function convertPdfBlobToBase64(pdfBlob) {
  return new Promise((resolve, reject) => {
    ...
    reader.onloadend = () => {
      ...
      // We need to remove the "data:application/pdf;base64," prefix.
...
export function sendToPrinter({ file, reportName, reportTitle, docNames },mimetype='pdf', proxyUrl = "http://127.0.0.1:12212/printer") {
        if (printService.isConnected()) {
            convertPdfBlobToBase64(file)
              .then((base64) => {
            printService.submit({ 'type': reportName, 'url': docNames+'.pdf' ,'file_content':base64 })  ;
            })
           .catch((error) => {
               console.error('Error converting PDF Blob to Base64:', error);
            downloadFile(file, docNames, mimetype);
             });
        } else {
            downloadFile(file, docNames, mimetype);
        }
...
function handleResponse(response, reportType) {
    ...
    return response
        ...
        .then(async (blob) => {
            ...
            const proxyUrl = `http://127.0.0.1:12212/printer`
            return sendToPrinter(printData, mimetype,proxyUrl)
        });
}
