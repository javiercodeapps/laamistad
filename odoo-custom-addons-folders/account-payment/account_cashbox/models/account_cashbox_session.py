    ...
    def _check_session_balance(self):
        for rec in self.filtered(lambda x: x.state == 'closed'):
            for line in rec.line_ids.filtered(lambda c: c.journal_id.id in rec.cashbox_id.cash_control_journal_ids.ids):
                ...
                diff_ss = line.balance_end - line.balance_end_real
                autorizo = self.env.context.get('autorizo_cierre')
                balance_cierre = self.env.context.get('balance_cierre')
                if balance_cierre:
                    diff_ss = line.balance_end - balance_cierre
                if (diff > max_diff_in_currency) and (not autorizo):
...
#                    account_loss = self.env['account.account'].search([('name', 'ilike', 'pérdida')], limit=1)
#                    account_gain = self.env['account.account'].search([('name', 'ilike', 'ganancia')], limit=1)
#                    if not account_loss or not account_gain:
#                        raise UserError("Debe existir una cuenta con nombre que contenga 'pérdida' y otra con 'ganancia'.")
                    ...
                    move_vals = {
                        'ref': f"{tipo} Cierre Sesión Caja {self.name}",
                    ...
                    if diff_ss < 0:
                        move_vals['line_ids'] = [
                            (0, 0, {'account_id': account_journal.id, 'name': 'Ganancia por Caja', 'credit': 0.0, 'debit': diff_ss}),
                            (0, 0, {'account_id': account_gain.id, 'name': 'Ganancia por Caja', 'credit': diff_ss, 'debit': 0.0}),
                        ]
                    else:
                        move_vals['line_ids'] = [
                            (0, 0, {'account_id': account_loss.id, 'name': 'Pérdida por Caja', 'credit': 0.0, 'debit': -diff_ss}),
                            (0, 0, {'account_id': account_journal.id, 'name': 'Pérdida por Caja', 'credit': -diff_ss, 'debit': 0.0}),
                        ]
