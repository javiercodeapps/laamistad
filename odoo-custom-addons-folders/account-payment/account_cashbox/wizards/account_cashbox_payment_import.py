...
class AccountCashboxPaymentImport(models.TransientModel):
    ...
    def default_get(self, field_list):
        ...
        if 'cashbox_session_id' in field_list:
            pop_session = self.env['account.cashbox.session'].browse(self.env.context.get('active_id'))
            res['cashbox_session_id'] = pop_session.id

            journal_ids = pop_session.line_ids.mapped('journal_id').ids

            # opening_date = pop_session.opening_date - timedelta(hours=3)
            opening_date = pop_session.opening_date

            _logger.info("Importing payments with adjusted opening_date (UTC-3): %s", opening_date)

            payments = self.env['account.payment'].search([
                ('journal_id', 'in', journal_ids),
                ('create_date', '>', opening_date),
                ('state', '=', 'posted'),
